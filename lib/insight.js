...
Insight.prototype.getTransaction = function(txid, callback) {
  ...
  this.requestGet('/api/rawtx/' + txid, function(err, res, body) {
    if (err || res.statusCode !== 200) {
      return callback(err || res);
    }
...
Insight.prototype.broadcast = function(transaction, callback) {
  ...
  this.requestPost('/api/tx/send', {
    ...
    if (err || res.statusCode !== 200) {
      if (!err && body.indexOf('-27') > -1) {
        return callback(new errors.AlreadyBroadcastedError(body));
      }
...
Insight.prototype.getMedianTime = function(callback) {
  ...
  this.requestGet('/api/blocks?limit=11', function(err, res, body) {
    ...
    try {
      var medianBlockTime = getMedianTime(JSON.parse(body).blocks)
